...
parser = argparse.ArgumentParser(description="RAxML command generator");
...
parser.add_argument("-p", dest="path", help="The path to RAxML. Default: raxml", default="raxml");
...
parser.add_argument("-model", dest="model", help="The sequence evolution model.", default=False);
parser.add_argument("-bs", dest="bs", help="The number of BS replicates per gene tree.", default=False);
parser.add_argument("-threads", dest="threads", help="The number of threads to compute each gene tree.", type=int, default=1);
# Program options
...
if not args.model:
    sys.exit(" * Error RAX1: A -model must be specified.");
if args.bs:
    try:
        args.bs = int(args.bs);
    except:
        sys.exit(" * Error RAX2: -bs must be an integer.");
    if args.bs < 1:
        sys.exit( " * Error RAX3: -threads must be a positive integer.");
if args.threads and args.threads < 1:
    sys.exit( " * Error RAX4: -threads must be a positive integer.");
# RAxML error checking
...
output_file = os.path.join(cwd, "jobs", "raxml_cmds_" + name + ".sh");
submit_file = os.path.join(cwd, "submit", "raxml_submit_" + name + ".sh");
...
with open(output_file, "w") as outfile:
    core.runTime("#!/bin/bash\n# RAxML command generator", outfile);
    ...
    core.PWS("# ----------", outfile);
    core.PWS("# RAxML OPTIONS", outfile);
    core.PWS(core.spacedOut("# Model:", pad) + args.model, outfile);
    if args.bs:
        core.PWS(core.spacedOut("# Number bootstrap reps:", pad) + str(args.bs), outfile);
    core.PWS(core.spacedOut("# Number threads per gene:", pad) + str(args.threads), outfile);
    core.PWS("# ----------", outfile);
...
# Generating the commands in the job file.
    ...
    for f in os.listdir(args.input):
        ...
        cur_outfile = os.path.join(args.output, base_input + "-raxml-" + name + ".fa");
        cur_logfile = os.path.join(logdir, base_input + "-raxml-" + name + ".log");

        seed = str(random.randint(1000000,999999999))
        # Generate the starting seed for RAxML.

        if args.bs:
            bs_seed = str(random.randint(1000000,999999999))
            # Generate the starting bootstrap seed for RAxML.

        raxml_cmd = "'" + args.path + "' ";
        if args.bs > 0:
            raxml_cmd += "-f a ";
        raxml_cmd += " -m " + args.model + " -p " + seed;
        if args.bs > 0:
            raxml_cmd += " -x " + bs_seed + " -# " + str(args.bs);
        if args.threads > 1:
            raxml_cmd += " -T " + str(args.threads);
        # if constraint_tree:
        #     raxml_cmd += " -g '" + constraint_tree + "'";
        #     if estimate_bl:
        #         raxml_cmd += " -f e";
        raxml_cmd += " -s '" + cur_infile + "' -n '" + base_input + "' -w '" + os.path.abspath(cur_outfile) + "'";
        raxml_cmd += " > " + cur_logfile + " 2>&1";

        outfile.write(raxml_cmd + "\n");
