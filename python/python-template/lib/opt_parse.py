...
# Defaults are set in params.py
    ...
    parser.add_argument("-i", dest="input_file", help="Whatever the input is.", default=False);
    parser.add_argument("-d", dest="input_dir", help="Whatever the input is.", default=False);
    parser.add_argument("-f", dest="other_file", help="Another optional input.", default=False);
    ...
    globs = inputPathCheck(args.input_file, "file", True, "-i", globs, "input-file");
    # Input file check

    globs = inputPathCheck(args.input_file, "dir", True, "-d", globs, "input-dir");
    # Input directory check

    globs = inputPathCheck(args.other_file, "file", True, "-f", globs, "other-file");
    # Other file check
    ...
    globs['output-file'] = os.path.join(globs['outdir'], globs['run-name'] + ".txt");
...
# A nice way to start the program.
    ...
    CORE.printWrite(globs['logfilename'], globs['log-v'], CORE.spacedOut("# Input file:", pad) + globs['input-file']);
    CORE.printWrite(globs['logfilename'], globs['log-v'], CORE.spacedOut("# Input directory:", pad) + globs['input-dir']);
    ...
    #                 CORE.spacedOut(globs['other-file'], opt_pad) + 
    #                 "A description of what the other file does.");
    # else:
    #     CORE.printWrite(globs['logfilename'], globs['log-v'], CORE.spacedOut("# -f", pad) +
    #                 CORE.spacedOut("NA", opt_pad) + 
    #                 "A description of what the other file does.");         
    # # Reporting the optional file   

    CORE.printWrite(globs['logfilename'], globs['log-v'], CORE.spacedOut("# -p", pad) +
                CORE.spacedOut(str(globs['num-procs']), opt_pad) + 
                "Script will use this many processes in parallel");  
    ...
    if globs['overwrite']:
        CORE.printWrite(globs['logfilename'], globs['log-v'], CORE.spacedOut("# --overwrite", pad) +
                    ...
                    "Script will OVERWRITE the existing files in the specified output directory.");
