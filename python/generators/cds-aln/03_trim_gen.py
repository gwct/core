...
# Generating the commands in the job file.
    ...
    for f in os.listdir(args.input):
        ...
        cur_outfile_nt = os.path.join(args.output, base_input + ".trim.fa");
        cur_out_csv = os.path.join(suppdir, base_input + "-stats.csv");
        cur_logfile = os.path.join(logdir, base_input + "-macse.log");

        macse_cmd = "java -jar ~/bin/macse_v2.03.jar -prog trimAlignment -align " + cur_infile + " -out_NT " + cur_outfile_nt + " -out_trim_info " + cur_out_csv + " > " + cur_logfile + " 2>&1";
