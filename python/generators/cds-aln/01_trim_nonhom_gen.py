...
suppdir = os.path.join(args.output, "supp");
# Job files
...
with open(output_file, "w") as outfile:
    ...
    core.PWS(core.spacedOut("# Supp file directory:", pad) + suppdir, outfile);
    if not os.path.isdir(suppdir):
        core.PWS("# Creating supp file directory.", outfile);
        os.system("mkdir " + suppdir);
...
# Generating the commands in the job file.
    ...
    for f in os.listdir(args.input):
        ...
        cur_outfile_nt = os.path.join(ntdir, base_input + ".NT.trim.fa");
        cur_outfile_aa = os.path.join(aadir, base_input + ".AA.trim.fa");
        cur_out_mask = os.path.join(suppdir, base_input + "-mask-detail.fa");
        cur_out_csv = os.path.join(suppdir, base_input + "-stats.csv");
        cur_logfile = os.path.join(logdir, base_input + "-trimnonhom.log");

        macse_cmd = "java -jar ~/bin/macse_v2.03.jar -prog trimNonHomologousFragments -seq " + cur_infile + " -out_NT " + cur_outfile_nt + " -out_AA " + cur_outfile_aa + " -out_mask_detail " + cur_out_mask + " -out_trim_info " + cur_out_csv + " > " + cur_logfile + " 2>&1";
